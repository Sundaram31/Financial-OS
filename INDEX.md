...
| Debt & Loan Tracker | `/loans/` | Live, auto-detects from bank statement, paste/CSV/Excel file upload; app-wide font-size/contrast pass (2026-08-11) also fixed a missing `.btn.primary:hover` state this module had drifted from the rest of the app; statement paste now also detects month-name dates (e.g. "01-Jan-2026"), and a real amount-misread bug (a date's own year digits could be picked up as a payment amount) found and fixed (2026-08-11); second amount-misread fix (2026-08-13) — a statement's own running-balance figure could get silently picked over the real EMI/prepayment amount, now excluded via label/DR-CR-marker/position signals before the amount is chosen, honestly skipping genuinely ambiguous lines; also fixed the scan-feedback message never actually rendering (same stale-DOM-node bug class as Portfolio's 2026-08-09 fix) | 2026-08-13 |
